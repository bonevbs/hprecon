function P = build_hprecon(elim_tree)
  root = find(elim_tree.fathers == -1);
  assert(isscalar(root), 'More than one root has been found.')
  [P,~] = build_hprecon_rec(elim_tree, root);
end

function [P, ncreated] = build_hprecon_rec(elim_tree, i)
  P = hprecon();
  n1 = 0; n2 =0;
  
  if (elim_tree.lsons(i) ~= -1)
    [P.Son1, n1] = build_hprecon_rec(elim_tree, elim_tree.lsons(i));
  end
  if (elim_tree.rsons(i) ~= -1)
    [P.Son2, n2] = build_hprecon_rec(elim_tree, elim_tree.rsons(i));
  end
